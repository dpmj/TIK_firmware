 ...
 * @file Button.h
 * @brief  
 * THIS IS A COPY OF THE BUTTON EXTENSION FROM THE TFT_eSPI LIBRARY
 * It's modified to have square buttons 
 * See Buttons.cpp
 * 
 */
...
class TFT_eSPI_Button_Mod : public TFT_eSPI {

 public:
  TFT_eSPI_Button_Mod(void);
  // "Classic" initButton() uses centre & size
  void     initButton(TFT_eSPI *gfx, int16_t x, int16_t y,
  uint16_t w, uint16_t h, uint16_t outline, uint16_t fill,
  uint16_t textcolor, char *label, uint8_t textfont);
